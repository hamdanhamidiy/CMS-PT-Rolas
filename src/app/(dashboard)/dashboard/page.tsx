      ...
      <div className="space-y-3">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Link href="/screens">
              <button className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold border border-slate-200/80 shadow-2xs transition-all">
                <MonitorPlay className="w-3.5 h-3.5 text-slate-500" />
                Kelola Layar
              </button>
            </Link>
            <Link href="/media/upload">
              <button className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-xs font-semibold shadow-2xs transition-all active:scale-[0.98]">
                ...
                {filteredScreens.map((screen) => (
                  <tr key={screen.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="px-6 py-3.5 text-right">
                      {/* Black button for Buka Player */}
                      <Link
                        href={`/player?code=${screen.screen_code}`}
                        target="_blank"
                        className="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-[11px] font-semibold transition-all shadow-2xs active:scale-[0.97]"
                      >
