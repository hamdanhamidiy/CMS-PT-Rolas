      ...
      <header className="flex flex-col gap-3 bg-white p-4.5 rounded-xl border border-slate-200/90 shadow-2xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap sm:flex-nowrap">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="h-9 px-3 text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-400 transition-all cursor-pointer flex items-center justify-between gap-2 min-w-[160px]">
                <div className="flex items-center gap-1.5 truncate">
                  <Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                  <span className="truncate">
                    {datePreset === 'all' && !startDateFilter && !endDateFilter
                      ? 'Semua Tanggal'
                      : datePreset === 'today'
                      ? 'Hari Ini'
                      : datePreset === '7days'
                      ...
                      ? `${startDateFilter || '...'} s/d ${endDateFilter || '...'}`
                      : 'Filter Tanggal'}
                  </span>
                </div>
                {(startDateFilter || endDateFilter) && (
              ...
              <DropdownMenuContent align="start" side="bottom" className="w-72 p-3 space-y-3">
                <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                  <span className="text-xs font-bold text-slate-900">Filter Periode Tanggal</span>
                  {(startDateFilter || endDateFilter) && (
                    <button
                      type="button"
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`py-1.5 px-2.5 rounded-lg text-xs font-bold transition-all border text-center ${
                        datePreset === p.id && !startDateFilter && !endDateFilter && p.id === 'all'
                          ? 'bg-slate-900 text-white border-slate-900 shadow-2xs'
                          : datePreset === p.id && (startDateFilter || endDateFilter)
                ...
                <div className="space-y-2 pt-2 border-t border-slate-100">
                  <span className="text-[11px] font-bold text-slate-700 block">Kustom Rentang Tanggal</span>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <span className="text-[10px] font-medium text-slate-400 block mb-1">Dari (Mulai)</span>
                      <input
                        type="date"
                        ...
                        className="w-full h-8 px-2 text-[11px] font-mono bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:outline-none"
                      />
                    </div>
                    <div>
                      <span className="text-[10px] font-medium text-slate-400 block mb-1">Sampai (Selesai)</span>
                      <input
                        type="date"
                        value={endDateFilter}
                        onChange={(e) => {
            ...
            {/* Reset Filters */}
            {(selectedActions.length > 0 || search || startDateFilter || endDateFilter) && (
              <button
        ...
        {(selectedActions.length > 0 || startDateFilter || endDateFilter) && (
          <div className="flex flex-wrap items-center gap-1.5 bg-blue-50/60 border border-blue-100 px-3.5 py-2 rounded-xl text-xs mt-1">
            ...
            {/* Date Filter Badge */}
            {(startDateFilter || endDateFilter) && (
              <span className="inline-flex items-center gap-1.5 text-[11px] font-bold text-blue-700 bg-white border border-blue-200 px-2.5 py-0.5 rounded-lg shadow-2xs">
                <Calendar className="w-3 h-3 text-blue-600" />
                <span>
                  Tanggal: {startDateFilter ? formatDate(startDateFilter) : 'Awal'} – {endDateFilter ? formatDate(endDateFilter) : 'Akhir'}
                </span>
                <button
                  onClick={() => handleDatePresetChange('all')}
